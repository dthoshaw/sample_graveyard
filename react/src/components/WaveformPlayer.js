...
export default function WaveformPlayer({ src, name }) {
  ...
  useEffect(() => {
    ...
    ctx.fillStyle = "rgba(129,140,248,0.2)"; // accent overlay
  ...
  return (
    <>
      <button
        ...
        className="text-accent hover:text-[#a5b4fc] mr-2"
      >
        {playing ? (
      ...
      <span className="text-xs text-light font-mono truncate max-w-[160px] mr-2">{name}</span>
      <canvas
        ref={canvasRef}
        width={180}
        height={20}
        className="w-44 h-5 bg-[#181818] rounded cursor-pointer"
        onClick={handleSeek}
      />
      <audio
        ref={audioRef}
